...
export const HotkeyItem = ({ hotkey, onEdit, isEditing, onSave, onCancel, onToggle }: HotkeyItemProps) => {
      ...
      <div className="py-3 space-y-3 border-b border-border last:border-0">
        ...
        <div className="flex gap-3">
          ...
          <Button
            ...
            aria-label={t("account.hotkeys.recordAria")}
          >
            {keyRecordingMode ? (
              <span className="text-primary-content font-medium animate-pulse">{t("account.hotkeys.pressNow")}</span>
            ) : editedKey ? (
              <KeyboardKey>{editedKey}</KeyboardKey>
            ) : (
              <span className="text-neutral-content-subtler">{t("account.hotkeys.setShortcut")}</span>
            )}
          </Button>
  ...
  return (
    ...
    >
      ...
      <Tooltip title={t("account.hotkeys.edit")}>
        <div
