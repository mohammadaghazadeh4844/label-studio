...
export const StorageCard = ({ rootClass, target, storage, onEditStorage, onDeleteStorage, storageTypes }) => {
  ...
  return (
    <Card
      header={storageData.title ?? t("settings.storage.untitled", { type: storageData.type })}
      extra={
        <Dropdown.Trigger
          align="right"
          content={
            <Menu size="compact" style={{ width: 110 }}>
              <Menu.Item onClick={() => onEditStorage(storageData)}>{t("settings.storage.edit")}</Menu.Item>
              <Menu.Item onClick={() => onDeleteStorage(storageData)}>{t("common.delete")}</Menu.Item>
            </Menu>
          }
        >
          <Button look="string" className="-ml-3" aria-label={t("settings.storage.optionsAria")}>
      ...
      <div className={rootClass.elem("sync")}>
        <div className="mt-base">
          <Button
            ...
            aria-label={t("settings.storage.syncAria")}
          >
            {t("settings.storage.sync")}
          </Button>
          {notSyncedYet && (
            <div className={rootClass.elem("sync-count")}>
              {t("settings.storage.syncingNote")}
            </div>
          )}
        </div>
      </div>
